...
我们添加了强大的文件管理功能，以增强您的 AI 体验！

#### **知识库功能：**

- **📁 文档管理** - 上传、创建、编辑和删除知识文档
- **📄 多格式支持** - 支持 .txt、.md、.docx、.pdf 和 .doc 文件
- **🔄 自动转换** - .docx、.pdf 和 .doc 文件自动转换为 .md 格式以兼容 AI
- **✏️ 应用内编辑** - 直接在浏览器中编辑 .txt 和 .md 文件
- **🎯 持久化上下文** - 知识库文件在所有会话中始终可被 AI 访问

#### **临时文件上传：**

- **💬 会话专属文件** - 为每个聊天会话上传特定文件
- **📂 代理文件系统访问** - 文件存储在代理的文件系统的 `temp_uploads/` 目录
- **🔧 工具集成** - AI 可以使用文件工具读取和处理上传的文件
- **🚀 即时可用** - 文件立即可用，无需重新加载代理
- **🗑️ 简便管理** - 一键删除临时文件
...
**知识库：**
1. 导航到**知识库**页面
2. 上传文档（.txt、.md、.docx、.pdf、.doc）或创建新的文本文件
3. 直接在浏览器中编辑内容
4. AI 自动可访问所有知识库文档

**临时文件（聊天）：**
1. 在聊天页面，点击输入框旁边的**添加文件**按钮
2. 选择要上传的文件（最大 10MB，格式与知识库相同）
3. 文件作为标签显示在输入框上方
4. AI 可以在会话期间通过文件系统工具访问文件
